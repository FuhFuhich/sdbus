...
class AppConfig {
   ...
   public:
    AppConfig(sdbus::IConnection& connection, const std::string& app_name, std::map<std::string, sdbus::Variant> config,
        ...
        object_->registerMethod("ChangeConfiguration")
            ...
            .implementedAs(
                [this](const std::string& key, const sdbus::Variant& value) { this->changeConfiguration(key, value); });
    ...
    std::map<std::string, sdbus::Variant> getConfiguration() { return config_; }
